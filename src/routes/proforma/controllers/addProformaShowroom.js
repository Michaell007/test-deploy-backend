...
export default async ({ bodymen: { body }}, res, next) => {
    try {
        ...
        if (showroom == null){
            return res.sendUserError("Cette showroom n'existe pas");
        }
        ...
        for (let i = 0; i < body.detailsSell.length; i++) {
            ...
            detailsElementShowroom[i] = await DetailsProforma.create({
            ...
            await proforma.update({
                montantTotalRemise: proforma.montantTotalRemise + (((parseInt(body.detailsSell[i].quantite) * prix.intitule) * detailsElementShowroom[i].remise) / 100)
            });

            await proforma.update({
                montantTotalHT: proforma.montantTotalHT + detailsElementShowroom[i].montant - proforma.montantTotalRemise
            });
